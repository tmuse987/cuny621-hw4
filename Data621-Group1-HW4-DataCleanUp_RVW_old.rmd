...
#TJ Edit
InsTrain <- read.csv("insurance_training_data.csv",na.strings="",header=TRUE)#, stringsAsFactors=FALSE)
InsEval <- read.csv("insurance-evaluation-data.csv",na.strings="",header=TRUE)#, stringsAsFactors=FALSE)
...
As revealed earlier there were a list of columns that were factors that should be integers. We start by 
...
older_instrain <- InsTrain
new_InsTrain<-InsTrain

older_instrain[,colnames(older_instrain) %in% c]<-data.frame(sapply(older_instrain[,colnames(older_instrain) %in% c], function(x) as.numeric(x)))
if(c %in% colnames(new_InsTrain)){

  new_InsTrain[1,]
  # https://r.789695.n4.nabble.com/Converting-dollar-value-factors-to-numeric-td2130536.html
  # This way negative quantities and numbers without dollar signs are handled correctly. 
  
  new_InsTrain[,c] <- sapply(new_InsTrain[,(colnames(new_InsTrain) %in% c)], 
...
This reduces the predictors used tp 25 from 30. The AIC improvesfrom 7482.66 (our original general model) to 7478.5, and we also benefit by having a simpler model less prone to overfitting.
...
The plot on the right shows that the number of predictors with the lowest BIC are `PARENT` , `HOMEVAL`, `CAR_USE`, 'CAR_TYPE', 'REVOKED', 'MVR_PTS' and 'URBANICITY'. We will use those predictors to build the next model

```{r echo=FALSE,message=FALSE,warning=FALSE}
m4 <- glm(TARGET_FLAG ~ PARENT1 + HOME_VAL + CAR_USE + CAR_TYPE + REVOKED + MVR_PTS + URBANICITY, family=binomial, data = InsTrain)
