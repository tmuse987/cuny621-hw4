...
The result is not a very good model showing a very low $R^2$. We also observe several parameters which are not very significant. We try a second model without these parameters, although we do not expect it so be much better that this first model.

**Model 2: significant predictors**

```{r echo=FALSE}
lm1<-lm(TARGET_AMT~KIDSDRIV+PARENT1+HOME_VAL+MSTATUS+EDUCATION+TRAVTIME+CAR_USE+TIF+CAR_TYPE+CLM_FREQ+REVOKED+MVR_PTS+CAR_AGE+URBANICITY,InsTrain)
summary(lm1)
par(mfrow = c(2,2))
plot(lm1)
```

As expected this model isn't any better than the first one. It is simpler, but its performance is basically the same. What we do notice, and very visible in the Q-Q plot, is that these seem to be a high number of data points distance from the normal. This suggest a different kind of model.

**Model 3: weighted coefficients**

```{r}
lm3<-rlm(TARGET_AMT~.-TARGET_FLAG,InsTrain)
summary(lm3)
```
